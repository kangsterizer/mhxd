...
# ifdef __powerpc__
#  define store_le32(r, addr) \
	__asm__ __volatile__("stwbrx %1,0,%2" : "=m" (*addr) : "r" (r), "r" (addr))
# else
#  define store_le32(r, addr) (*(addr) = SWAP(r))
# endif
#else
# define SWAP(n) (n)
# define store_le32(r, addr) (*(addr) = SWAP(r))
...
md5_read_ctx (ctx, resbuf)
     ...
     void *resbuf;
{
  store_le32(ctx->A, &((md5_uint32 *)resbuf)[0]);
  store_le32(ctx->B, &((md5_uint32 *)resbuf)[1]);
  store_le32(ctx->C, &((md5_uint32 *)resbuf)[2]);
  store_le32(ctx->D, &((md5_uint32 *)resbuf)[3]);
     ...
     void *resbuf;
{
  ...
  store_le32(ctx->total[0] << 3, (md5_uint32 *)&ctx->buffer[bytes + pad]);
  store_le32((ctx->total[1] << 3) | (ctx->total[0] >> 29),
...
#define BLOCKSIZE 4096
  ...
  while (1)
	...
	do
	  {
	    n = read (fd, buffer + sum, (BLOCKSIZE - sum) > len ? len : (BLOCKSIZE - sum));
